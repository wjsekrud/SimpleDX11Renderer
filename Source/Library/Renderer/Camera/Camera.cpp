...
void Camera::Update(_In_ FLOAT deltaTime)
{
    ...
    m_at = XMVector3TransformCoord(DEFAULT_TARGET, m_rotation);
    ...
    if (m_character != nullptr) {
        m_eye = m_character->GetCurrentPosition() - XMVectorSet(0.0f, -15.0f, 25.0f, 0.0f);
        m_at = m_character->GetCurrentPosition();
    }
...
void Camera::SetCharacter(_In_ const std::shared_ptr<Character>& character) {
    ...
    m_eye = XMVectorSet(0.0f, 15.0f, -25.0f, 0.0f);
    m_at = XMVector3TransformCoord(DEFAULT_TARGET, m_rotation);
}